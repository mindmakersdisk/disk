  ...
  Http.onreadystatechange = (e) => {
    ...
    try {
      ...
      if (retorno && retorno != null) {
        try {
          document.getElementById("login").value = 'usuario_e' + retorno.user;
        } catch (e) {}
...
function comutePsw(img) {
  ...
  if (img.src.indexOf('eye_checked') > -1) {
    ...
    try {
      pswField2 = document.getElementById('password2');
      pswField2.type = "password"
    } catch (e) {};
    img.src = "img/eye.png";
  } else {
...
function canChangePsw() {
  ...
  try {
    document.getElementById('changePsw').style.display = "inline";
  } catch (e) {}
...
function enviaAlerta(msg) {
  var notification = alertify.notify(msg, 'success', 8, function() {
      ...
      if (retorno.status == "OK") {
        ...
        if (indiceGanho == 0) {
          ...
          var notification = alertify.notify(decodificaNotificacao(ganhoPersonal[ganho]), 'custom', 10,
            function() {
              setTimeout(verificaSeClicouGanho, 100, indiceGanho);
            });
...
function iniciaDisputaFinal() {
  ...
  for (var key of estacoes.keys()) {
    ...
    if (estacao.grupo && estacao.grupo != '' && estacao.grupo != '0') {
      ...
      if (estacao.grupo == '1')
        if (estacao.papel == 'u') eq1TotUsu++;
        else eq1TotHac++;
      else if (estacao.grupo == '2')
        if (estacao.papel == 'u') eq2TotUsu++;
        else eq2TotHac++;
      else if (estacao.grupo == '3')
        if (estacao.papel == 'u') eq3TotUsu++;
        else eq3TotHac++;
      else if (estacao.grupo == '4')
        if (estacao.papel == 'u') eq4TotUsu++;
        else eq4TotHac++;
      else if (estacao.grupo == 'i')
        if (estacao.papel == 'u') indTotUsu++;
        else indTotHac++;
