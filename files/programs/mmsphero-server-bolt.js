...
*/
          console.log('\x1b[32m', 'Sphero Bolt registrado com sucesso! ');
          // Modifica o atalho e variável
          atualizaAtalhoSphero();
          setTimeout(encerraAposLeitura, 10000);
  }
...
function procurarNovoSphero() {
  ...
  console.log('Procurando por um Sphero BOLT próximo para configurar...');
  ...
  noble.on('stateChange', function(state) {
    if (state === 'poweredOn') {
      ...
      console.log('Procurando por Sphero BOLT a menos de 20cm...');
      noble.startScanning();
    } else {
...
noble.on('discover', function(peripheral) {
  ...
  if (peripheral.rssi > -75 && (
      ('' + peripheral.advertisement.localName).indexOf('SB') == 0 ||
      ('' + peripheral.advertisement.localName).indexOf('Sphero') == 0)) {

    console.log('\x1b[32m', 'Encontrou Sphero BOLT:' + peripheral.address + ' [Nome:' + peripheral.advertisement.localName +
  ...
  if (numeroScans >= 8 && global_jsondevicelist.length == 0) {
    ...
    console.log('Não localizei um Sphero BOLT próximo. Para registrar, aproxime uma unidade a menos de 20cm do computador.');
    console.log('');
    noble.stopScanning();
    ...
    if (numeroTentativas == 1) {
      ...
      inquirer.prompt(questionsInformar).then(answers => {
        ...
        if (answers.macaddress == undefined || answers.macaddress == null || answers.macaddress == '') {
          ...
          console.log('Vai configurar Sphero BOLT nesta estação com macaddress informado: ' + answers.macaddress);
      ...
      console.error('\x1b[31m', 'Não foi possível encontrar um Sphero BOLT para registrar.');
