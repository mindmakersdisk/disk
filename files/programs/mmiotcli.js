...
// Ex: http://localhost:800/imgs/sala/cockpit1.png
...
function exibeImagem(request,response,imagemComPath,definitiva) {
    ...
    shell.exec("sudo fbi -T 10 --noverbose -t 10 --once -a "+imagemComPath, function(code, output) {
      if(code!=0) {
       ...
       response.json({
        ...
        console.log('definitiva4');
        shell.exec("sudo fbi -T 10 --noverbose -a "+imagemComPath, function(code, output) {
      if(code!=0) {
        response.json({
          error: "Erro ao tentar exibir imagem "+imagemComPath
       })
